...
export function renderTrendsPanel(ctx: PanelContext): PanelInstance {
  ...
  const seriesVisible: Record<string, boolean> = {};
  for (const dim of DIMENSIONS) {
    seriesVisible[dim.key] = true;
  }

  // WS listener for readiness snapshots
  ...
  function renderContent(
    data: ReadinessApiResponse['data'],
  ): void {
    if (!data.current) {
      showEmptyState();
      return;
    ...
    if (data.history.length > 0) {
      ...
      p.textContent =
        'Run bootstrap at least twice to see trend data.';
      p.style.color = 'var(--color-text-muted)';
      noHistory.appendChild(p);
      chartContainer.appendChild(noHistory);
    }
  ...
  function createGauge(label: string, percent: number): HTMLElement {
    ...
    // Fill arc: proportional to percentage
  ...
  function renderSvgChart(
    ...
    chartElement = svg as unknown as HTMLElement;
    ...
    for (const dim of DIMENSIONS) {
      ...
      polyline.style.display = seriesVisible[dim.key] ? 'block' : 'none';
      svg.appendChild(polyline);

      // Data point circles (invisible until hover)
      for (let i = 0; i < history.length; i++) {
        ...
        circle.style.display = seriesVisible[dim.key] ? 'block' : 'none';
  ...
  function renderCanvasChart(
    ...
    // Draw series
    for (const dim of DIMENSIONS) {
      if (!seriesVisible[dim.key]) continue;
  ...
  function renderLegend(): void {
    ...
    for (const dim of DIMENSIONS) {
      ...
      item.style.opacity = seriesVisible[dim.key] ? '1' : '0.3';
      ...
      item.addEventListener('click', () => {
        seriesVisible[dim.key] = !seriesVisible[dim.key];
        item.style.opacity = seriesVisible[dim.key] ? '1' : '0.3';

        // Toggle polyline visibility in SVG chart
        if (chartElement && chartElement.tagName === 'svg') {
          const svgEl = chartElement as unknown as SVGSVGElement;
          const elements = svgEl.querySelectorAll(`[data-series="${dim.key}"]`);
          for (const el of elements) {
            (el as SVGElement).style.display = seriesVisible[dim.key]
              ? 'block'
              : 'none';
          }
        } else {
          // Canvas: need to re-render
          loadData();
        }
      });
