...
/** DOM elements created by buildHeatmapDom. */
interface HeatmapDom {
  wrapper: HTMLElement;
  sortBar: HTMLElement;
  gridArea: HTMLElement;
}

/** Mutable state for the heatmap panel. */
interface HeatmapState {
  currentSort: SortMode;
  dirGroups: DirGroup[];
  sortButtons: Record<SortMode, HTMLButtonElement>;
}
...
function buildHeatmapDom(container: HTMLElement): HeatmapDom {
  ...
  return { wrapper, sortBar, gridArea };
}
...
function updateSortButtons(state: HeatmapState): void {
  for (const [mode, btn] of Object.entries(state.sortButtons)) {
    if (mode === state.currentSort) {
...
function sortGroups(groups: DirGroup[], currentSort: SortMode): DirGroup[] {
  const sorted = [...groups];
  switch (currentSort) {
    case 'worst':
...
function renderCells(
  ...
  if (sorted.length === 0) {
    showHeatmapEmptyState(gridArea);
    return;
  }
...
function showHeatmapEmptyState(gridArea: HTMLElement): void {
  gridArea.innerHTML = '';
...
function showHeatmapErrorState(gridArea: HTMLElement, message: string): void {
  gridArea.innerHTML = '';
  const empty = document.createElement('div');
  empty.className = 'empty-state';
...
export function renderHeatmapPanel(ctx: PanelContext): PanelInstance {
  ...
  // Bind sort controls
  bindSortControls(dom.sortBar, state, () => {
    renderCells(dom.gridArea, state.dirGroups, state.currentSort);
  });
  ...
  async function loadData(): Promise<void> {
    try {
      ...
      state.dirGroups = transformHeatmapData(filesMap);
      renderCells(dom.gridArea, state.dirGroups, state.currentSort);
    } catch (err: any) {
      if (err.status === 404) {
        showHeatmapEmptyState(dom.gridArea);
      } else {
        showHeatmapErrorState(dom.gridArea, err.message || 'Failed to load conventions');
      }
    }
  }
