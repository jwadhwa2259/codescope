...
export function renderCommandPanel(ctx: PanelContext): PanelInstance {
  ...
  let fileList: string[] = [];
  const searchInstances: Array<{ destroy: () => void }> = [];
  ...
  // WS listener for graph updates (refresh button feedback)
  ...
  const cardDefs: CardDef[] = [
    {
      ...
      action: (card, button) => {
        showFileSearch(card, button, async (filePath) => {
      ...
      action: (card, button) => {
        showFileSearch(card, button, async (filePath) => {
          ...
          try {
            ...
            // Show floating badge with affected count
  ...
  for (const def of cardDefs) {
    grid.appendChild(createCard(def));
  }

  function createCard(def: CardDef): HTMLElement {
  ...
  function showFileSearch(
    ...
    // Check if search is already shown
    ...
    const instance = renderSearch(searchContainer, fileList, (item) => {
