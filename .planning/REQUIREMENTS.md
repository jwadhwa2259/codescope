...
- [ ] **GRAPH-03**: Schema migration adds ON DELETE CASCADE to edges table preventing dangling references
- [ ] **GRAPH-04**: SQLite busy_timeout pragma (5000ms) enables safe concurrent access between MCP server and dashboard
...
- [ ] **SESS-01**: `/codescope:pause` generates structured handoff document with completed work, remaining tasks, key decisions, and resume command
- [ ] **SESS-02**: `/codescope:resume` reads handoff document and resumes orient pipeline at the correct phase/wave
...
- [ ] **VIZ-01**: Local HTTP server (Hono) on port 7463 serves single-page dashboard with JSON API endpoints
...
- [ ] **DIST-01**: `npx codescope init` detects project, creates config, runs bootstrap, shows "what you got" summary
- [ ] **DIST-02**: CLI entry point with subcommands: init, bootstrap, viz, review, install-hooks, status
- [ ] **DIST-03**: Plugin auto-setup configures `.claude-plugin/plugin.json` and `.mcp.json` if Claude Code detected
